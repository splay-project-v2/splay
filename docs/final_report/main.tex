...
\begin{document}
  ...
  \chapter{Introduction}
    ...
    \section{Learning Distributed Algorithms}
      ...
      classic algorithms (consensus, leader election, resource allocation, ...) and if ever they want
  ...
  \chapter{Problem Statement - Splay}
    ...
    \section{Roadmap}
      ...
      \subsection{Changes}
        ...
        % TODO : the project need changes/refactor for maintability reasons
      ...
      \subsection{New Features}
        ...
        % TODO : quick explication of enhancement of SPLAY
...
...
          ...
          candidate will become leader and only one leader in the same time. 
        ...
        \subsection{Animation results}
          ...
          A complete raft election can be chopped in multiple steps. The next figure shows a complete
          election with a conflict between three candidates (candidates asking almost in the same time
          vote requests to the others). In this case, nobody get the majority of vote, then a other election
          will be trigger with a bigger term at some point. And with this new election, the candidate will received
          enough vote and become the leader (no conflit this time). Note that, conflicts happen rarely with 
          3 candidates because the leader timeout is well balanced randomizely and the topology used 
          is not a extreme case (topology 2 \ref{fig:topo2}).
          ...
          \begin{figure}[H]
            \begin{subfigure}{.45\textwidth}
              ...
              \caption{When all nodes is awaken and they are all follower (begin state)}
              \label{fig:ele1}
            \end{subfigure}\hspace{0.1\textwidth}
            \begin{subfigure}{.45\textwidth}
              \centering
              \includegraphics[width=1.0\linewidth]{figures/user_case/election_2.png}
              \caption{Three candidate for the leader job -> conflict, none become the leader, the majority is sliped}
              \label{fig:ele2}
            \end{subfigure}
            \begin{subfigure}{.45\textwidth}
              \centering
              \includegraphics[width=1.0\linewidth]{figures/user_case/election_3.png}
              \caption{New leader election from job 4 with a bigger term -> became the leader when received the majority}
              \label{fig:ele3}
            \end{subfigure}\hspace{0.1\textwidth}
            \begin{subfigure}{.45\textwidth}
              \centering
              \includegraphics[width=1.0\linewidth]{figures/user_case/election_4.png}
              \caption{The leader (job 4) sending heartbeat to other nodes to avoid new leader election}
              \label{fig:ele4}
            \end{subfigure}
          
            \caption{Raft election animation from the parsing log of a job with the topology in figure \ref{fig:topo2} 
            and crashes point for the leader}
            \label{fig:election}
          \end{figure}

          In this case, not one node had crash, but we can show that even when crash happend, the raft is fault-tolerant as long as 
          there are the majority of node alive (3 here), if there only one or two nodes alive the election can't terminated until a third wakes up.
          The animation shows that when the leader crashed, one or multiple nodes 
          restart a new the leader election (a upgrade the current term). \\
          Crashes can also happen on follower or candidate node without perturbate the raft process (when mojority is on, of course).
      ...
      \section{Buggy code proof}
        ...
        After some search, he finds a way to create a example where the  therefore executes this algorithm implementation
