...
\begin{document}
  ...
  \chapter{Introduction}
    ...
    \section{Splay}
      ...
      topologies, \textsc{SplayNet}~\cite{SplayNet}. This middleware implements
    ...
    \section{Our Work on Splay}
      ...
      As their Github repository explains :
      \begin{displayquote}
        "INGInious is an intelligent grader that allows secured and automated
        testing of code made by students."
      \end{displayquote}
      We think that our work is complementary to what INGInous achieves,
      ...
      of \texttt{Splay} in chapter~\cite{chap:newfeat}, present a real scenario
      of usage in chapter~\cite{chap:usercase} and then conclude this document.
  ...
  \label{chap:splayteaching}
    ...
    \section{What Splay Offered}
      ...
      In this section, we will go through specific topics concerning the Splay
      ...
      \subsection{Application Usage}
        ...
        \textbf{\texttt{job}} encapsulates everything needed by the application
        to run the distributed algorithm and is composed of the following
        elements:

        \begin{itemize}
          \item The distributed algorithm, in Lua, that the user wants to test.
          ...
          \textsc{SplayNet}~\cite{SplayNet}, the user can design a network
          \textbf{\texttt{topology}} which will be used by the application
      ...
      \subsection{Software Architecture}
        ...
        \begin{figure}[H]
          ...
          \caption{\label{prev_arch} Legacy Splay Architecture}
        \end{figure}
        ...
        \subsubsection{CLI and Web application}
          ...
          (connection, registration), sending new \textbf{\texttt{jobs}},
          requesting \textbf{\texttt{logs}} and getting informations
          ...
          \begin{itemize}
            \item The CLI server is written in Ruby, running a simple
            HTTP server and waiting for specific HTTP calls containing
            commands representing the wanted interaction (new job, session
            creation, ...).
            \item The CLI client is written in Lua and is composed of
            multiple Lua files, each one representing a command to send
            to the server.
          \end{itemize}

          The web application is written in Ruby on Rails~\cite{ror}, and
        ...
        \subsubsection{Controller}
          ...
          The \textbf{\texttt{controller}}, written in Ruby, is the service
          ...
          use of \textsc{SplayNet} and emulate the network conditions.\\
          This service is also responsible for the \textbf{\texttt{logs}}
          creation.

        \subsubsection{Daemons}

          The \textbf{\texttt{daemons}} are the last piece of the system.
          Written in Lua, they are not Linux daemons, but services designed
          to take the role of nodes on which the \textbf{\texttt{jobs}} will
          run.\\
          The \textbf{\texttt{daemons}} register on the
          \textbf{\texttt{controller}} through TCP connections so that they
          can communicate.\\

          The creators of Splay made the choice of Lua because it is a very
    ...
    \section{User Scenarios}
      ...
      \subsubsection{A student trains on Splay}
      ...
      \subsubsection{A professor trains on Splay}
        ...
        Lua code in a dedicated editor, create special cases of topologies
    ...
    \section{Roadmap}
      ...
      \subsection{Changes}
        ...
        \subsubsection{Testing Implementation}
          ...
          It was totally impossible to be sure that our changes weren't
        ...
        \subsubsection{Front-end Refreshment}
          ...
          promoted by Rails such as REST~\cite{} or keeping lightweight
          controllers~\cite{fatmode}.\\
      ...
      \subsection{New Features}
        ...
        \subsubsection{Complete Lua editor on the website}
          ...
          \begin{itemize}
            ...
            \item Code coloration for the Lua language in the editor.
        ...
        \subsubsection{Topology creator/visualization}
          ...
          used in \cite{ModelNet}, \texttt{Splay} users previously
        ...
        \subsubsection{Fault Injection}
          ...
          most straightforward way to achieve thisis to let
  ...
  \label{chap:dev}
    ...
    of the system.\\
    We'll first go through more detailed explanations on the base \texttt{Splay}
    ...
    \section{The Base Project}
      ...
      \subsection{Architecture}
        ...
        The old \texttt{Splay} architecture was composed of:

        \begin{itemize}
          \item The \textbf{Controller}: The core part of \texttt{Splay}, waits for jobs
          and dispatch them to the daemons.
          \item \textbf{Daemons}: Workers registering to the controller and waiting
          for jobs to achieve.
          \item A \textbf{MySQL DB}: The communication piece of \texttt{Splay}, allowing
          ...
          \texttt{Splay} through communication with the MySQL Database.
        \end{itemize}

        Whether through the usage of the CLI application or the web application,
        the user was able to interact with the \texttt{Splay} system, to send his
        jobs, and gather many information such as the logs or the current
        state of the running \texttt{Splay}ds (the \texttt{Splay} daemons).\\
        ...
        \textbf{Controller} is the central piece within the \texttt{Splay} project,
        ...
        We thus chose to operate major changes on this part of the \texttt{Splay}
        project, in order to combine the arrival of our new features and
        changes onto the core of \texttt{Splay} with better user experience.
    ...
    \section{Project Analysis and Modifications}
      ...
      \subsection{Github Repository Reorganization}
        ...
        From our analysis on the \texttt{Splay} project, we weren't satisfied with how
    ...
    \section{Backend Development}
      ...
      \subsection{The Choice of Rails}
        ...
        therefore make the future evolution of \texttt{Splay} easier.\\
    ...
    \section{Web Application Development}
      ...
      \subsection{The new front-end application development}
        ...
        A list of \texttt{Splay} daemons (created by the controller) is retrieved
    ...
    \section{CLI Development}
      ...
      \subsection{Using Python}
        ...
        in the days, and to avoid scattering \texttt{Splay} around too much different
    ...
    \section{Bug fix on Daemon and Controller}
      ...
      among \texttt{Splay} that weren't assigned a complete refactor because of how
      ...
      \subsection{Daemons changes}
        ...
        and also made some improvements on the \texttt{Splay}'s Lua library.
  ...
  \label{chap:qual}
    ...
    \section{Testing and Validation}
      ...
      \subsection{Testing in Daemons}
        ...
        provides a rich distributed library (\texttt{Splay} lib) written in Lua to the user.\\
        ...
        (small functionalities) of the Lua \texttt{Splay} library. Some of those tests
        ...
        Sadly the whole \texttt{Splay} lib of the Daemon isn't entirely tested,
...
2019-05-30 08:20:10.0622 (2)  3: RPC Append entry FROM 2.0 Term: 1.0 Entry: null
        ...
        \begin{itemize}
          \item Line 1 : the leader timeout is finish on the node 2, then a leader election is trigger.
          \item Lines 2-5 : the candidate (node 2) send vote request to other nodes
          \item Lines 6-13 : others nodes received the vote request from node 2 and return the result.
          \item Lines 14-16 : node 2 received the two result of its vote requests and obtain the majority of vote.
          Then the node becomes the leader.
          \item Lines 17-22 : Leader (node 2) send heartbeat directly to other nodes to specify that it is the leader now
          (also he receives the rest of vote request response).
          \item Lines 23-26 : Others nodes gets heartbeat from the leader and reset their election timeout. 
        \end{itemize}
    ...
    \section{Robustness of solution}
      ...
      parts of the process. Moreover, he wants to see the difference of chance to become leader
      depending of the network topology.

      \subsection{Network Topology Emulation}

        The student wants to tests it implementation on different network topology
        and see the difference on the chance for each node to become leader.
        ...
        \subsubsection{Network topology impact}
          ...
          The student then tries his implementation of with three network configurations~\ref{fig:topo1}~\ref{fig:topo2}~\ref{fig:topo3}.
          ...
          But the network topology can affect 
          the leader selection (some nodes have more chance to other becoming the leader), he expects
          to obtain different results for each topology :
          \begin{itemize}
            \item First topology \ref{fig:topo1} : he expects that each node have almost the same chance to be the leader because 
            the latencies between endpoints is small compared to the timeout election range (1.5 seconds).
            \item Second topology \ref{fig:topo2} : he expects than daemon 3-4-5 have slightly more chance 
            to become the leader (a kind of partition is done with a majority and two other node) than daemon 1-2 because 
            there is a kind of partition between the first two nodes and the rest with a big latency (250 ms).
            \item Thrid topology \ref{fig:topo3} : he expects that the daemon 5 has very small chance to become the leader compared to the others.
            Also we can see that the communication with daemon 5 create always 
          ...
          the student makes experiments with single fault injection. he adds 
          a crash point in the heartbeat function to crash the leader before the second heartbeat and trigger a new election : 
          \begin{lstlisting}[style=MyLua]
...
function heartbeat()
...
...
          ...
          The student launch the code turn during approximately 13 minutes and parse the log to get 
          the number of time each node is became leader. First, he concludes that crashing the leader
          does not crash the Raft election process but only restart a election as expected. \\
          Then, the result~\ref{tab:leader} after the long run supports 
          his expectations concerning the chance of each node to become the leader.\\
          Here, the purpose is \textbf{not} making statistical evidence/model but 
          only showing the importance of the topology emulation on a distributed algorithm
          and specifically on the chance of each node to become the leader during a Raft election process.
          But we can imagine a advanced study to clearfy the impact of the network to the leader 
          election (or other process) with the help of Splay. 
          ...
          \begin{table}[]
            ...
            \multicolumn{1}{|l|}{} & Node 1                 & Node 2                & Node 3         & Node 4        & Node 5                & Total leader election \\ \hline
            Topology 1, \ref{fig:topo1}           & 20.36 \% (90)          & 21.27 \% (94)         & 19.91 \% (88)  & 18.78 \% (83) & 19.68 \% (87)         & 100 \% (442)          \\ \hline
            Topology 2, \ref{fig:topo2}           & \textbf{10.50 \% (38)} & \textbf{9.67 \% (35)} & 28.17\% (102)  & 25.69 \% (93) & 25.97 \% (94)         & 100 \% (362)          \\ \hline
            Topology 3, \ref{fig:topo3}            & 22.61 \% (90)          & 25.63 \% (102)        & 25.63 \% (102) & 23.62 \% (94) & \textbf{2.51 \% (10)} & 100 \% (398)          \\ \hline
            \end{tabular}
      ...
      \subsection{Advanced fault injection}
        ...
        The user have already test the triggering of election in the previous section
        by crashing the leader but he wants to increase the confidence of is implementation. 
        Then, he add other fault injection at different key point of his algorithm to 
        see how his implementation will manage different faults.

        To achieve this, the student puts four crash points in his code.
        First ad before, he adds a crash point in the heartbeart function but with other parameters
        (when the 6th heartbeart is called and recover after 0.5 second) : 
        \begin{lstlisting}[style=MyLua]
...
function heartbeat()
...
...
        \end{lstlisting}
        Secondly, he adds crash points not depending on the
        node's state but triggering less often. Firstly, when a RPC vote
        request is received, this one will be triggered by a node when it receives a vote request
...
...
        ...
        Secondly, he adds a crash point when a vote is granted by a node :
        \begin{lstlisting}[style=MyLua]
...
        if vote_granted == true then
          nb_vote = nb_vote + 1
...
...
        ...
        And last, he creates a fault injection just \textbf{after} than a node reply to a vote request RPC.
        \begin{lstlisting}[style=MyLua]
...
        set_election_timeout() -- reset the election timeout
    end
...
...
        \end{lstlisting}
        We add a new thread and adding the crash in it, it is a trick for that the crash append after the return 
        and the RPC reply on the network. It is working because the Non-preemptive of Lua will first finish the current 
        code before passing to a other coroutine (behind the scene of the thread). \\

        After letting running the code on 5 daemon during 10 minutes,
        he checks the logs for the different topology
        settings, and he sees that everything went as expected. Indeed,
        leader elections happen without any problem (if majority on) and there is no more than one leader
        in the same time. Moreover, when there are not enough nodes alive (less than 3), 
        the leader election will not progress until the majority of nodes is on. \\
        He does not conclude the correctness of his code, because it is impossible to test every 
        situation, but he is more confident about the Raft election process and 
        his pratical code of it. 
