    ...
    \section{Bug fix on Daemon and Controller}
      ...
      \subsection{Controller changes}
        ...
        \begin{itemize}
          ...
          more readable (issue of the old project \cite{logTimestamps}). % REMYWHAT
...
-- CRASH POINT 1 2 :RECOVERY 1 : TIME 3
    ...
    \chapter{Use Case}
      ...
      Secondly, with the help of Splay library, he builds a basic raft election implementation. Thirdly, he 
      wants to test with differents network topology and after analyse the robustness of his solution by creating
      some fault injection at key points. Also, one of his friend send to him a slightly different version
      avoiding some writings on disk, and he wants to prove, by a example, the illness of the solution.
      And for conclude his tests on Raft, he wants show a animation of the evolution of the endpoint state. 
...
2019-05-29 12:57:13.7279 (6)  RPC Append entry FROM 2.0 Term : 1.0 Entry : null
      ...
      \section{Topology settings}
        ...
        even when the leader is still on, it is because the condition $broadcast time << election timeout$ is not met.

        {\color{red} TODO}

      \section{Robustness Test through fault injection}

        He wants to test the correctness of his code by making crash the leader essentialy, but also the others.
        When the leader crash, the student expects that a other leader election will be trigger by a remain node 
        , and then a new leader will appear.
        He tests with the 2 corrects topologies (don't change the timeout election).
