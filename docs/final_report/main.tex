  ...
  \chapter{Architecture and Changes Development}
    ...
    \section{Web Application Development}
      ...
      \subsection{The new front-end application}
        ...
        In this section, we will talk about the new modern web application with
        all old features.
    ...
    \section{CLI Development}
      ...
      \subsection{Developing the CLI}
        ...
        output of your CLI, displaying you help details if requested and even
        help details for specific commands.\\
    ...
    \section{Bug fix on Daemon and Controller}
      ...
      As explained, before only the daemon and the controller didn't get a complete refactor
      but obviously, these services weren't not stable and lot of bugs happened 
      during our tests. Then we presented all our change on these, sometimes
      to fix issues or only to have more maintainable code.

      \subsection{Controller changes} 
        We have fixed lot of issues on the controller, mainly because
        of sequel call. Then, you can see a no exaustive list of change on the controller :
        \begin{itemize}
          \item Sequel calls : we have seen that in the old source code, someone tried to change 
          the old library dbd-mysql \cite{dbdMysql} to Sequel \cite{Sequel} (issue in the old project \cite{dbdMysqlToSequel}).
           We are completely agree with this 
          change (dbd-mysql is not updated since 2010, have still bugs and Sequel manages different type of Database)
          , but most of the SQL calls through 
          Sequel was wrongly written, then lot of call returned nothing (select case) or insert nothing (insert case).
          We have checked every Sequel usage in the all code, and change when it was mandatory, also we tried to 
          clean as much as possible (but remaining a lot) raw string SQL call with interpolation (see the improvements section in the conclusion).
          \item Communicatation with daemon : the comminucation with daemons was not coherent with
          the daemon protocol. By example, the logs retrievement was buggy because of protocol not coherent between 
          the two services. Logs retrievement and creation has be improved to by more readable (issue of the old project \cite{logTimestamps}).
          \item Small changes : we added more logs when needed and removed some less useful. we removed the creation 
          of the database from the controller (backend). Also, we used Rubocop \cite{Rubocop} to have a certain logic
          in the code. 
        \end{itemize}

      \subsection{Daemons changes} 
        We changed/fixed small parts of daemon to be consistent with the controller (communication) and maked some improvements
        of Lua Splay library (again not exaustif): 
        
        \begin{itemize}
          \item As explain in the controller part, we fixed the protocol with the controller, mainly for the logs and 
          the registering of the daemon.
          \item The topology socket restriction is now automatically done when a topology is set. This restriction is used
          to simulate the toplogy settings, then limited the bandwidth and created the latency.
          The topology socket has a main issue in its design, indeed the restriction is based only on the ip and port (one), then 
          if the daemon used a client port (tcp or udp), the server will answer into a unknwon port without any restriction.
          Then to bypass of this issued, we created a new topology restriction based only on the ip, but it means that 
          all nodes (daemons) need to have a different ip (with docker-compose, it is the case). But obviously, it is 
          something to improve and change (check improvements section).
          \item Removed LuaCrypto library from the daemon and added luaossl to replace it. The main reasons were LuaCrypto can't use 
          the last openssl linux package (security issue and old version can't be used in new OS), 
          it was not updated anymore and a issue on this was open in the old project \cite{sslLib}. Then the code was updated depending of this update.
          \item Multiple bug fix due (mainly) to the change of version of Lua has be solved. 
        \end{itemize}
    ...
    \chapter{New Features}
      ...
      \section{Fault injection}
        ...
        \subsection{Domain Specific Language (DSL)}
          ...
          needed to be integrated into the Lua code without any modification of
          it (expected for rigthful crash comments), trying to avoid totally crashing
          the daemon in case of the crash points specified by the user were not correct.\\
          ...
          \begin{itemize}
            ...
            affected nodes separated by a space (ids begin at 1 -> number of daemon used).
...
<WHEN> => AFTER <int> | RANDOM <float>
          ...
          \begin{itemize}
            ...
            execute 5 times before triggering the crash. And if you want crash immediately when
            the line is execute for the first time, you can define AFTER 0.
            ...
            encounters the crash point through the code execution. By example,
            if you specified 1, the crash will occure in the first execution 
            (similar to AFTER 0).
          \end{itemize}
...
-- CRASH POINT 1 2 3 4 5 :RECOVERY 0:RANDOM 0.5
          ...
          Also, we added some negative examples (won't trigger any crash
...
-- CRASH POINT 1 2 :RECOVERY 1 : TIME 3
        ...
        \subsection{Parsing}
          ...
          The parsing phase actually assign, to the nodes choosed by the user through ids,
          a new code where the crash points comments is specified by the users.
          This new replaced (if no error is found) the crash comment by a function call 
          containing the crash id number (as parameters). Therefore, when the code executes, the crash function
          call will access the library and will act depending on what
          informations are stored into the crash table (type of crash, moment
          of trigger).
        ...
        \subsection{Execution}
          ...
          can be defined the down time, our simple solution of that is using 
          the non-preemptive feature of Lua and force to sleep x seconds without any coroutine yield
          (then other coroutines can't execute any code) before crashing.

    \chapter{Use Case} % TODO : Modify
      ...
      \section{Installation of Splay} % TODO : Modify
...
docker-compose -f docker-compose.prod.yml up -d --scale daemon=7
      ...
      \section{Implementation of Raft in Lua} % TODO : Modify
        After the successful installation, the main work need to be done, it means to create the implementation of Raft
        ...
        \subsection{Creation of the algorithm} % TODO : Modify
        ...
        \subsection{First result} % TODO : Modify
        ...
        and this one send hearbeat to avoid a other leader election from others nodes : 
          \begin{lstlisting}[style=MyBash]
2019-05-29 12:57:13.7139 (7)  Election Trigger - upgrade term and become candidate 
2019-05-29 12:57:13.7144 (7)  Send request to 1 - {"port":11691,"ip":"172.18.0.7"}
2019-05-29 12:57:13.7153 (7)  Send request to 3 - {"port":11691,"ip":"172.18.0.10"}
2019-05-29 12:57:13.7156 (7)  Send request to 4 - {"port":11691,"ip":"172.18.0.12"}
2019-05-29 12:57:13.7158 (7)  Send request to 5 - {"port":11552,"ip":"172.18.0.13"}
2019-05-29 12:57:13.7174 (3)  RPC Request Vote FROM 2.0 Term : 1.0
2019-05-29 12:57:13.7174 (3)  Stepdown : 1.0 > 0
2019-05-29 12:57:13.7184 (6)  RPC Request Vote FROM 2.0 Term : 1.0
2019-05-29 12:57:13.7184 (6)  Stepdown : 1.0 > 0
2019-05-29 12:57:13.7193 (4)  RPC Request Vote FROM 2.0 Term : 1.0
2019-05-29 12:57:13.7193 (4)  Stepdown : 1.0 > 0
2019-05-29 12:57:13.7212 (1)  RPC Request Vote FROM 2.0 Term : 1.0
2019-05-29 12:57:13.7214 (1)  Stepdown : 1.0 > 0
2019-05-29 12:57:13.7242 (7)  Vote Request result 1.0 : true from 3
2019-05-29 12:57:13.7242 (7)  Vote Request result 1.0 : true from 5
2019-05-29 12:57:13.7242 (7)  I Become the LEADER now
2019-05-29 12:57:13.7246 (7)  Vote Request result 1.0 : true from 4
2019-05-29 12:57:13.7248 (7)  Send append entry (null) to 1
2019-05-29 12:57:13.7249 (7)  Send append entry (null) to 3
2019-05-29 12:57:13.7251 (7)  Send append entry (null) to 4
2019-05-29 12:57:13.7251 (7)  Send append entry (null) to 5
2019-05-29 12:57:13.7260 (7)  Vote Request result 1.0 : true from 1
2019-05-29 12:57:13.7265 (3)  RPC Append entry FROM 2.0 Term : 1.0 Entry : null
2019-05-29 12:57:13.7269 (1)  RPC Append entry FROM 2.0 Term : 1.0 Entry : null
2019-05-29 12:57:13.7274 (4)  RPC Append entry FROM 2.0 Term : 1.0 Entry : null
2019-05-29 12:57:13.7279 (6)  RPC Append entry FROM 2.0 Term : 1.0 Entry : null
          \end{lstlisting}

      \section{Topology settings} % TODO : Modify

        Atfer testing the algorithm in a perfect network environment, the student tries with 3 different configurations
        of network topology created by the network topology interface developed by us : 
        ...
        \begin{figure}[H]
          ...
          \begin{subfigure}{.33\textwidth}
            \centering
            \includegraphics[width=1.0\linewidth]{figures/user_case/raft_topo_fail.png}
            \caption{1c}
            \label{fig:topo3}
          \end{subfigure}
          \caption{plots of topologies}
      ...
      \section{Robustness Test through fault injection} % TODO : Modify
      ...
      \section{Result with a animation} % TODO : Modify
        ...
        \subsection{Timeline Animation of a graph evolution} % TODO : Modify
          % REM
          {\color{red} TODO}

        \subsection{Result with raft} % TODO : Modify
          % For any case, he wants to see the evolution of the nodes graph during the execution by parsing the log.
          {\color{red} TODO}

  \chapter{Conclusion} % TODO : Finish
    ...
    \section{Improvements} % TODO : Check

      \subsection{New features improvements}

        \subsubsection{Lua editor} % TODO : Check
        ...
        \subsubsection{Crash point} % TODO : Check
        ...
        \subsubsection{Topology Creation} % TODO : Check
      ...
      \subsection{Refactor of old services} % TODO : Check
        ...
        \subsubsection{Daemon} % TODO : Check
          ...
          Moreover, we seen at the end of the project that the socket topology restriction don't take in account the packet 
          loss rate and the queue length (maybe the github code is not really the last one). 
          Then, the topology settings is limited to the bandwidth and the latency. Furthermore, we have seen 
          (in the section of daemon changement) that the topology restriction is based on the ip + a single port, causing 
          unkowning client socket (because port is different than the server port) and useless restriction.
          For these reason the the socket topology (coming with SplayNet module) need to be improved. \\
        ...
        \subsubsection{Controller} % TODO : Check
      ...
      \subsection{Security aspect} % TODO : Check
